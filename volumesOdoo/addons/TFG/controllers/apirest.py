        ...
        #Si se ha indicado id, hay busqueda
            if dicDatos:
                for key in dicDatos:
                    if key == "director":
                        a = [0, 2]
                        search = [('director', '=', int(dicDatos["director"]))]
                        ele = "d"
                    elif key == "id":
                        search = [('id', '=', int(dicDatos["id"]))]
                        ele = "i"
                    elif key == "genero":
                        search = [('genero', '=', int(dicDatos["genero"]))]
                        ele = "g"
            else:
                return "{'estado':'CARGAMENTO NO INDICADO'}"
            ...
            if (http.request.httprequest.method == 'GET'):
                ...
                if record and record[0] and ele == "i":

                    for s in record:
                        imgurl = base_url + '/web/image?' + 'model=peliculas&id=' + str(s.id) + '&field=portada'

                        lista_peliculas.append({'id':s.id,'nombre':s.nombre,'director': s.director.nombre , 'dirID': s.director.id,

                        'sinopsis':s.sinopsis,'genero':s.genero.nombre, 'genID':s.genero.id, 'imagen':s.imagen,'trailer':s.trailer, 'puntuacion':s.puntuacion, 'visitas':s.visitas, 'img':imgurl})

                    return http.Response( 
                    json.dumps(lista_peliculas, default=str)[1:-1], 
                        status=200,
                        mimetype='application/json'
                    )
                elif record and record[0] and (ele == "d" or ele == "g"):
                    var = []
                    for x in record:
                        var.append(x.read())
                    json_result = http.Response(json.dumps(var, default=str)[1:-1].replace("], [",","),  status=200, mimetype='application/json')
                    return json_result
                else:
                    return "{'estado':'NO ENCONTRADO'}"
        elif model == "comentarios": 
            search = []
            #Pasamos lo recibido en "data" a un diccionario
            dicDatos=json.loads(args['data'])
            #Si se ha indicado id, hay busqueda
            ...
            if dicDatos:
                for x in dicDatos:
                    if x == "id":
                        ...
                        search = [('pelicula', '=', int(dicDatos["pelicula"]))]
                        ele = "p"
            else:
                return "{'estado':'CARGAMENTO NO INDICADO'}"
            #Si es GET, devolvemos el registro de la busqueda
            if (http.request.httprequest.method == 'GET'):
                record = http.request.env[model].sudo().search(search)
                lista_peliculas =[]
                if record and record[0] and ele == "p":
                    var = []
                    for x in record:
                        var.append(x.read())
                    json_result = http.Response(json.dumps(var, default=str)[1:-1].replace("], [",","),  status=200, mimetype='application/json')
            ...
            if (http.request.httprequest.method == 'GET'):
                ...
                lista_peliculas =[]
                if record and record[0] and ele == "p":
                    if dicDatos["contrasenya"] == record.contrasenya:
                        var = []
                        for x in record:
                            var.append(x.read())
                        json_result = http.Response(json.dumps(var, default=str)[1:-1].replace("], [",","),  status=200, mimetype='application/json')
                        print("niceee "+record.contrasenya)

                        return "existe"
                    else:
                        print(record.contrasenya)
                        return False
